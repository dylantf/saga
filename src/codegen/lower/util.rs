...
/// Peel a chain of App nodes to find an EffectCall head and its collected arguments.
/// Returns (op_name, qualifier, args) if the head is an EffectCall.
pub(super) fn collect_effect_call(expr: &Expr) -> Option<(&str, &Option<String>, Vec<&Expr>)> {
    let mut args: Vec<&Expr> = Vec::new();
    let mut current = expr;
    loop {
        match current {
            ...
            Expr::EffectCall {
                ...
                args.reverse();
                return Some((name.as_str(), qualifier, args));
            }
            _ => return None,
        }
    }
