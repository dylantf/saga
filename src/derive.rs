...
fn derive_record_stringify(
    ...
    let where_clause: Vec<TraitBound> = type_params
        ...
        .map(|tp| TraitBound {
            type_var: tp.clone(),
            traits: vec![TraitRef { id: NodeId::fresh(), name: trait_name.into(), type_args: vec![], span: Span { start: 0, end: 0 } }],
        })
        .collect();
...
fn derive_ord(
    ...
    let where_clause: Vec<TraitBound> = type_params
        ...
        .map(|tp| TraitBound {
            type_var: tp.clone(),
            traits: vec![TraitRef { id: NodeId::fresh(), name: "Ord".into(), type_args: vec![], span: Span { start: 0, end: 0 } }],
        })
        .collect();
