...
#[test]
fn named_effect_annotation_without_bonus_param_is_error() {
    let result = check(
        "effect State s {\n\
         fun get : Unit -> s\n\
         }\n\
         fun transfer : Unit -> Int needs {from: State Int}\n\
         transfer () = from.get! ()",
    );
    assert!(result.is_err());
    let err = result.err().unwrap();
    assert!(
        err.message.contains("not a handler binding"),
        "expected missing handler binding error, got: {}",
        err.message
    );
}

#[test]
fn named_effect_bonus_param_without_annotation_is_inferred() {
    check(
        "effect State s {\n\
         fun get : Unit -> s\n\
         }\n\
         ...
         transfer () {from} = from.get! ()\n\
         main () = {\n\
         handle src = make_state\n\
         transfer () with { from: src }\n\
         }",
    )
    .unwrap();
}
...
#[test]
fn unused_named_effect_annotation_emits_warning() {
    let mut checker = check(
        "effect State s {\n\
         fun get : Unit -> s\n\
         }\n\
         fun transfer : Unit -> Unit needs {from: State Int}\n\
         transfer () {from} = ()",
    )
    .unwrap();
    ...
    assert!(
        checker.collected_diagnostics.iter().any(|d| {
            matches!(d.severity, Severity::Warning)
                && d.message.contains("function 'transfer' declares needs")
        }),
        "expected unused named effect warning, got: {:?}",
        checker.collected_diagnostics
    );
}

#[test]
fn named_effect_use_without_annotation_or_bonus_param_is_error() {
    let result = check(
        "effect State s {\n\
         fun get : Unit -> s\n\
         }\n\
...
#[test]
fn named_effect_annotation_and_bonus_param_name_mismatch_is_error() {
    let result = check(
        "effect State s {\n\
         fun get : Unit -> s\n\
         }\n\
    ...
    assert!(
        result.is_err(),
        "expected annotation/bonus-param mismatch to fail"
    );
}

#[test]
fn used_named_instance_param_is_not_unused_variable() {
    let mut checker = check(
        "effect Log {\n\
         fun log : String -> Unit\n\
         }\n\
         fun greet : String -> Unit needs {out: Log}\n\
         greet name {out} = out.log! name",
    )
    .unwrap();
    ...
    assert!(
        !checker
            .collected_diagnostics
            .iter()
            .any(|d| d.message.contains("unused variable: `out`")),
