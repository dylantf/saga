...
**Bridging the two systems:** Effect absorption (in `infer.rs` App handling) uses shallow row resolution (`resolve_var` + `apply_effect_row_shallow`) to only absorb explicitly declared effects from a parameter annotation, not effects captured by a row variable. This means row-captured effects correctly remain in `EffectState.current` and propagate to the caller, keeping the string-based tracking in sync with the type-level row bindings for the common case. The two systems are still technically parallel, but the absorption fix eliminates the main source of divergence.
...
- **Two-tier effect tracking:** The string-based `EffectState.current` and the type-level `EffectRow` with row variables are parallel systems. The main synchronization point (effect absorption using shallow row resolution) keeps them consistent for the common HOF case. Remaining gap: `commit_callee_effects` still reads from the static `fun_effects` map and doesn't consult resolved row variables, so `check_undeclared_effects` on the caller side may not see all effects. This hasn't caused issues in practice since the absorption fix handles the critical path.
...
The effect system is now a hybrid: string-based tracking for the common case (declaring and checking `needs` on named functions) and row-polymorphic types for the higher-order case (forwarding unknown effects through callbacks). The two systems are bridged at effect absorption time (shallow row resolution prevents row-captured effects from being incorrectly swallowed). The remaining architectural debt is that `commit_callee_effects` doesn't consult resolved row variables, but this hasn't caused practical issues since absorption handles the critical path correctly.
