...
| Trait method value, pure | `let f = show` after dict elaboration | Supported for direct monomorphic calls and direct HOF callback values when the concrete dict is known | Known dispatcher lambdas partially applied to known dicts lower to direct closures; dynamic dict method values still use runtime extraction | Broaden for imported/generic value canaries as needed |
...
| Trait method values | Specialize `let f = method` separately from immediate calls | Known pure method values can now become direct closures for direct HOF callback positions; known CPS methods need explicit CPS adapter closures; dynamic dict methods still extract runtime closures |
