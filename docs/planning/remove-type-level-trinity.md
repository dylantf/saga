...
- **The replacement has already been validated.** The Kraken querybuilder was
  ...
  ~1,300 lines of pure desugaring with zero runtime machinery. The JSON library
  has an Elm-style codec version that works without derives. So the trinity buys
  "a bit of userland niceness" that we are deliberately trading away.
...
| --- | --- |
| `deriving (Generic)` + user-defined derives | Closed set of compiler derives (`src/derive/builtin.rs`); add new ones (e.g. `Json`) by direct codegen if demand is real |
| Auto JSON via Generic | Elm-style hand-written codecs (already shipped); optional future curated `deriving (Json)` |
...
All of this is recoverable from git history and the validated Kraken/JSON
spikes. Do the removal on a branch; if a load-bearing dependency surfaces
...
Do we keep a path to **curated `deriving (Json)`** (compiler-generated codegen,
like `Show`) as the sanctioned answer to "I want automatic JSON without writing
codecs"? It does not reintroduce any type-level machinery and is the natural
Rust-std-derive-shaped successor to Generic-based JSON. Recommended: yes, but as
a separate follow-up, not part of this removal.
